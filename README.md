...
- **PIR general settings** (WMS) — backoff time (cooldown in seconds after a motion event, 1–3600) and LED enable toggle are configured in device settings via `GET/POST /api/v1/settings/pir` (partial JSON body — only `backoff_time` and/or `led_enable` need to be included).
